...
static const char *percent_to_bar(const char *percent);
static const char *battery_bar(const char *bat);
...
static const char *ram_bar(void);
...
static const char *swap_bar(void);
...
static const char *vol_bar(const char *card);
static const char *vol_perc(const char *card);
static const char *wifi_bar(const char *iface);
...
static const char *
percent_to_bar(const char *percent_str)
{
	char *bar_str;
	int percent;
	int i;
	...
	bar_str = malloc(sizeof(char) * (bar_len + 1));
	if (bar_str == NULL)
		return unknown_str;
	for (i = 0; i < bar_len; i++) {
		if (i * 100 / bar_len < percent)
			bar_str[i] = bar_elem;
		else
			bar_str[i] = bar_empty;
	}
...
{
	const char *percent = battery_perc(bat);
	return percent_to_bar(percent);
}
...
static const char *
disk_bar(const char *mnt)
{
	const char *percent = disk_perc(mnt);
	return percent_to_bar(percent);
}
...
static const char *
vol_bar(const char *card)
{
	const char *percent = vol_perc(card);
	return percent_to_bar(percent);
}
...
static const char *
wifi_bar(const char *iface)
{
	const char *percent = wifi_perc(iface);
	return percent_to_bar(percent);
}
